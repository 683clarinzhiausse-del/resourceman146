let shoes = [
    { id: 1, name: "Urban Runner X1", price: "₱2,499", image: "https://picsum.photos/id/201/600/600", desc: "Lightweight daily trainer with excellent breathability and responsive cushioning." },
    { id: 2, name: "Gibson Elite Pro", price: "₱3,299", image: "https://picsum.photos/id/202/600/600", desc: "Premium safety shoe with steel toe and slip-resistant sole." },
    { id: 3, name: "Marikina Classic", price: "₱2,899", image: "https://picsum.photos/id/203/600/600", desc: "Handcrafted leather shoe from Marikina's finest artisans." }
];
 
let techProducts = [
    { id: 'tech-1', name: "Smart Insoles", price: "₱1,499", image: "https://picsum.photos/id/204/600/600", desc: "Bluetooth-enabled insoles that track your steps and posture." }
];
 
let shoelanders = [
    { id: 'sl-1', name: "Shoelander Premium", price: "₱3,999", image: "https://picsum.photos/id/205/600/600", desc: "Luxury shoelander with premium leather and custom fit technology." },
    { id: 'sl-2', name: "Shoelander Sport", price: "₱3,499", image: "https://picsum.photos/id/206/600/600", desc: "Athletic shoelander designed for performance and comfort." }
];
 
let marikinaCollection = [
    { id: 'mar-1', name: "Marikina OTOP Special", price: "₱2,799", image: "https://picsum.photos/id/207/600/600", desc: "One Town One Product - Marikina's finest footwear export." },
    { id: 'mar-2', name: "Marikina Heritage", price: "₱3,199", image: "https://picsum.photos/id/208/600/600", desc: "Traditional Marikina craftsmanship meets modern design." }
];
...
const API_BASE_URL = window.location.hostname === 'localhost'
    ? 'http://localhost:3001'
    : 'https://resourceman-server.683clarinzhiausse.workers.dev';

// Fallback: if Cloudflare Worker is not deployed, use localStorage only
const USE_LOCAL_STORAGE_ONLY = false; // Set to true if backend is not available
...
async function saveCustomerRegistry() {
    ...
    try {
        ...
        if (response.ok) {
            console.log('✅ Customers synced to server');
        } else {
            console.warn('⚠️ Server sync failed, but data saved locally');
        }
    } catch (e) {
        console.error("Sync failed, but data saved locally:", e);
    }
}
...
async function loadCustomerRegistry() {
    try {
        ...
        if (response.ok) {
            ...
            customerRegistry = Array.isArray(data) ? data : [];
            localStorage.setItem('resourceManCustomerRegistry', JSON.stringify(customerRegistry));
...
async function loadCatalog() {
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            // Only overwrite if server returned non-empty arrays (protect seed data)
            if (data.shoes && data.shoes.length > 0) shoes = data.shoes;
            if (data.techProducts && data.techProducts.length > 0) techProducts = data.techProducts;
            if (data.shoelanders && data.shoelanders.length > 0) shoelanders = data.shoelanders;
            if (data.marikinaCollection && data.marikinaCollection.length > 0) marikinaCollection = data.marikinaCollection;
            localStorage.setItem('resourceManCatalog', JSON.stringify(data));
            console.log("✅ Catalog loaded from server");
        } else {
            throw new Error("Server response not OK");
        }
    } catch (e) {
        console.warn("Offline fallback for catalog:", e);
        const saved = localStorage.getItem('resourceManCatalog');
        if (saved) {
            try {
                ...
                console.error("Failed to parse saved catalog");
            }
        }
    }
}
...
async function saveCatalog() {
    ...
    try {
        ...
        if (response.ok) {
            console.log('✅ Catalog synced to server');
        } else {
            console.warn('⚠️ Server sync failed, but data saved locally');
        }
    } catch (e) { 
...
window.navigateTo = function(sectionId) {
 ...
 // EmailJS credentials - Fixed Version
let EMAIL_JS_USER_ID = "ysem6TdGvgnoidL6g";
let EMAIL_JS_SERVICE_ID = "service_h4lqwoo";
let EMAIL_JS_TEMPLATE_ID = "template_13vuh2j";
let EMAIL_JS_VERIFICATION_SERVICE_ID = "service_h4lqwoo";
let EMAIL_JS_VERIFICATION_TEMPLATE_ID = "template_vcb7uhr";
let EMAIL_SUPPORT_EMAIL = "resourcemanenterprises88888@gmail.com";
 
function isEmailJsEnabled() {
    const uidOk = typeof EMAIL_JS_USER_ID === 'string' && EMAIL_JS_USER_ID.trim();
    const sidOk = typeof EMAIL_JS_SERVICE_ID === 'string' && EMAIL_JS_SERVICE_ID.trim();
    const tidOk = typeof EMAIL_JS_TEMPLATE_ID === 'string' && EMAIL_JS_TEMPLATE_ID.trim();
...
function initEmailJS() {
    if (window.emailjs && isEmailJsEnabled()) {
        try { window.emailjs.init({ publicKey: EMAIL_JS_USER_ID }); } catch (e) { console.warn('emailjs.init failed', e); }
    }
}
...
function sendVerificationEmail(email) {
    ...
    try {
        return window.emailjs.send(EMAIL_JS_VERIFICATION_SERVICE_ID, EMAIL_JS_VERIFICATION_TEMPLATE_ID, templateParams)
            .then(() => {
                statusEl.innerHTML = `<p style="margin:0;">Verification code sent to ${email}. Check your inbox (and spam folder).</p>`;
            })
            .catch(error => {
                console.error('EmailJS error', error);
                const errorMsg = error && error.text ? error.text : 'Unknown error';
                statusEl.innerHTML = `
                    <p style="margin:0;color:var(--danger);">Email sending failed: ${errorMsg}</p>
                    <p style="margin: 10px 0 0; color: var(--success-color); font-weight: 600;">Dev fallback code: ${verificationCode}</p>
                `;
...
function showEmailJsConfig() {
    const currentConfig = `
Current EmailJS Configuration:
- User ID: ${EMAIL_JS_USER_ID}
- Service ID: ${EMAIL_JS_SERVICE_ID}
- Template ID: ${EMAIL_JS_TEMPLATE_ID}
- Verification Service: ${EMAIL_JS_VERIFICATION_SERVICE_ID}
- Verification Template: ${EMAIL_JS_VERIFICATION_TEMPLATE_ID}
- Support Email: ${EMAIL_SUPPORT_EMAIL}

Is this correct? If not, use configureEmailJs() to update.
    `;
    alert(currentConfig);
}
...
function submitInquiry(event) {
    ...
    sendInquiryAutoReply(email, name, subject, message);
...
function sendInquiryAutoReply(customerEmail, customerName, subject, message) {
    if (!window.emailjs || !isEmailJsEnabled()) {
        console.log('EmailJS not configured - inquiry email not sent');
        return Promise.resolve();
    }
    ...
    try {
        return window.emailjs.send(EMAIL_JS_SERVICE_ID, EMAIL_JS_TEMPLATE_ID, templateParams)
            .then(() => {
                console.log('Auto-reply sent to customer:', customerEmail);
            })
            .catch(error => {
                console.error('Auto-reply email error:', error);
                const errorMsg = error && error.text ? error.text : 'Unknown error';
                console.warn('⚠️ EmailJS Error:', errorMsg);
                if (error && error.status === 400) {
                    console.warn('⚠️ Template not found. ID:', EMAIL_JS_TEMPLATE_ID);
                    console.warn('📧 Template variables should include: {{name}}, {{title}}');
                }
                return Promise.resolve();
            });
    } catch (error) {
        console.error('Auto-reply send threw synchronously:', error);
        return Promise.resolve();
    }
}
...
window.addEventListener('load', async () => {
    const API_BASE_URL = 'https://resourceman-server.683clarinzhiausse.workers.dev';
    ...
    // 🔥 Explicitly render all product grids now that data is loaded
    if (typeof renderShoes === 'function') renderShoes();
    if (typeof renderShoelanders === 'function') renderShoelanders();
    if (typeof renderMarikina === 'function') renderMarikina();
    if (typeof renderTechProducts === 'function') renderTechProducts();
    console.log('✅ All product grids rendered');
});
